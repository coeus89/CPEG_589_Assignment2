...
class DCGAN_MODEL(object):
    def __init__(self, channels: int, epochs: int, batch_size: int):
        print("DCGAN model initalization.")
        self.G = DCGAN.DCGAN_Gen(channels)
        self.D = DCGAN.DCGAN_Des(channels)
        self.G_filename = './DCGAN_generator.pkl'
        self.D_filename = './DCGAN_discriminator.pkl'
        self.img_filename = 'dgan_model_image.png'
        self.C = channels
        self.cuda_index = 0
        self.cuda = False
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, _) in enumerate(train_loader):
                ...
                if generator_iter % 1000 == 0:
                    ...
                    utils.save_image(grid, 'training_result_images/img_DCGAN_generatori_iter_{}.png'.format(str(generator_iter).zfill(3)))
                ...
                if ((i + 1) % 100) == 0:
                    ...
                    z = Variable(torch.randn(self.batch_size, 100, 1, 1).cuda(self.cuda_index)) # JK: Trying out next line
                    # z = Variable(torch.randn(self.batch_size, 100, 1, 1).to(self.device)) # JK: This works too
    ...
    def evaluate(self):
        ...
        utils.save_image(grid, self.img_filename)
    ...
    def save_model(self):
        ...
        torch.save(self.G.state_dict(), self.G_filename)
        torch.save(self.D.state_dict(), self.D_filename)
        print('Models save to ./DCGAN_generator.pkl & ./DCGAN_discriminator.pkl ')

    def load_model(self):
        D_model_path = os.path.join(os.getcwd(), self.D_filename)
        G_model_path = os.path.join(os.getcwd(), self.G_filename)
